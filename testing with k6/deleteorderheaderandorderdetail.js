...
export default function () {
   ...
   function randomNumber(min, max) {
    ...
    let order_details_id  = randomNumber(1,500) 
    const url = `http://localhost:1337/api/order-details/${order_details_id}`
    const params  = {
      headers: {
    ...
    check(res,{
        ...
        'items not found':(r) => errorRate.status !== 200,
      }) || errorRate.add(1)
    if(res.status==200){      
        const url = `http://localhost:1337/api/order-details/${order_details_id}`
        const params  = {
        ...
        const res = http.Delete(url,params)
        check(res,{
            'status is 200':(r) => r.status === 200,
            'items not found':(r) => errorRate.status !== 200,
          }) || errorRate.add(1)
